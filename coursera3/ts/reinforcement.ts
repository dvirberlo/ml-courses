...
export abstract class DecisionTable<Action> {
  ...
  public abstract set(hash: string, decision: QDecision<Action>): Promise<void>;
...
export class FileDecisionTable<Action> implements DecisionTable<Action> {
  ...
  public async get(hash: string): Promise<QDecision<Action> | undefined> {
    ...
    try {
      return JSON.parse(
        await Deno.readTextFile(`${this.dirPath}/${this.hashString(hash)}.json`)
      );
    } catch (e) {
      console.log(
...
export const multiPlayerTrain = async <Action>(
  ...
  const getReward = async (state: State<Action>): Promise<number> => {
    ...
    if (state.isTerminal()) return env.reverseReward(env.getStateReward(state));
    // const best = (
    //   await Promise.all(
...
module tree {
  ...
  export abstract class ActionRewardTree<Action> {
    // TODO
  }

  export class MemoryActionRewardTree<Action>
    implements ActionRewardTree<Action> {
    // TODO
  }

  export class FileActionRewardTree<Action>
    implements ActionRewardTree<Action> {
    // TODO
  }

  export const fillActionRewardTree = async <Action>(
    env: RewardTwoPlayerEnvironment<Action>,
    tree: ActionRewardTree<Action>,
    gamma = 1
  ): Promise<void> => {
    const actionIndexStack = [0];
    const stateStack = env.getInitialStates();
  };
}
...
export class BotPlayer<Action> extends Player<Action> {
  constructor(
    public readonly decisionTable: DecisionTable<Action>,
